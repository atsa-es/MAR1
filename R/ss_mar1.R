...
ss.mar1 <- function(aggregated.data, MAR.obj = NULL, model = list(), control = list(), silent = FALSE) {
  ...
  if (is.null(model$B)) {
    if (missing(MAR.obj)) {
      stop("The B matrix must be specified.\n", call. = FALSE)
    } else {
      model$B <- MAR.obj$bestfit$B
    }
  }
  ...
  if (is.null(model$C)) {
    if (missing(MAR.obj)) {
      stop("The C matrix must be specified.\n", call. = FALSE)
    } else {
      model$C <- MAR.obj$bestfit$C
    }
  }
  ...
  match.rows <- sapply(
    format(aggregated.data$date, date.format),
  ...
  if (!all(apply(marss.var, 1, function(x) {
    all.equal(mean(x, na.rm = TRUE), 0)
  }))) {
  ...
  if (n.cov > 0) {
    marss.model <- list(B = B, U = U, C = C, Q = Q, Z = Z, A = A, D = D, R = R, tinitx = 1, c = marss.cov)
  } else {
    marss.model <- list(B = B, U = U, Q = Q, Z = Z, A = A, D = D, R = R, tinitx = 1)
  } # C is zero
  marss.dat <- marss.var
  ...
  rownames(B.rtn) <- var.names
  colnames(B.rtn) <- var.names
  rownames(Q.rtn) <- var.names
  colnames(Q.rtn) <- var.names
  rownames(C.rtn) <- var.names
  colnames(C.rtn) <- cov.names
  rownames(R.rtn) <- var.names
  colnames(R.rtn) <- var.names
  rownames(A.rtn) <- var.names
  colnames(A.rtn) <- "a"
