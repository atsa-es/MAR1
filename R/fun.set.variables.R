fun.set.variables <- function(heads) {
  ...
  for (i in 2:length(track)) {
    ...
    assign(paste("tog", "b", i, sep = "."), tkbutton(frame.b,
      text = "   not included   ",
      bg = "gray90", foreground = "gray60"
    ))
  ...
  for (i in 1:length(heads)) {
    ...
    if (i == 1) {
      tkgrid(
        tklabel(frame.b, text = "    "), colnum, tklabel(frame.b, text = "   "), head, tklabel(frame.b, text = "    "),
  ...
  while (changeVal >= 0) {
    ...
    if (changeVal > 0) {
      ...
      if (track[changeVal] == 0) {
        b.lab <- "   not included   "
        textcol <- "gray60"
        bgcol <- "gray90"
      }
      if (track[changeVal] == 1) {
        b.lab <- "        variate         "
        textcol <- "blue3"
        bgcol <- "gray75"
      }
      if (track[changeVal] == 2) {
        b.lab <- "      covariate       "
        textcol <- "firebrick"
        bgcol <- "gray75"
      }
      tkconfigure(get(paste("tog.b", changeVal, sep = ".")), text = b.lab, foreground = textcol, bg = bgcol)
    } # ;print(track)
  }
