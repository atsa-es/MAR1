plot.MAR <- function(x, y = NULL, ..., legend = FALSE) {
  ...
  if (any(unlist(lapply(
    all.list,
    function(x) {
      if (inherits(x, "MAR")) TRUE else FALSE
    }
  )) == FALSE)) {
  ...
  dots <- substitute(c(...))
  if (missing(y)) {
    argnames <- c(all.names(substitute(x)))
  } else {
  ...
  for (i in 1:length(all.list)) {
    arrayres[, , i] <- all.list[[i]]$restrictions.set
  }
  ...
  matBC <- cbind(
    c(0, 0, nlabs[(ncol(BC) + 1):length(nlabs)], 0),
    matBC, 0
  )
  ...
  mycol <- c(
    "#E6E6E6", "#FFE6CC", "#CCFFCC", "#BFE6FF", "#F2FFFF",
    "#FFE6FF", "#FFFFCC"
  )
  palette(mycol)
  ...
  for (i in 1:nrow(B)) {
    for (j in 1:ncol(B)) {
      barplot(rev(arrayB[i, j, ]),
        horiz = T, axes = F, space = c(0, 0),
      ...
      colresB <- resBij
      colresB[colresB == 1] <- "forestgreen"
      colresB[colresB == 0] <- "red"
      pchresB <- resBij
      pchresB[pchresB != .5] <- 18
      points(
        x = rep(0, length(argnames)), y = rev((1:length(argnames)) - .5),
  ...
  if (length(C) > 0) {
    for (i in 1:nrow(C)) {
      for (j in 1:ncol(C)) {
        ...
        colresC <- resCij
        colresC[colresC == 1] <- "forestgreen"
        colresC[colresC == 0] <- "red"
        pchresC <- resCij
  ...
  text(.5, 1, expression(bold(B) * "-matrix"),
    adj = 0,
  ...
  if (length(C) > 0) {
    plot(1, 1, type = "n", axes = F, xlim = c(.5, 1.5))
    text(.5, 1, expression(bold(C) * "-matrix"),
      adj = 0,
      font = 2, cex = 1.4, col = "black"
    )
  }
  ...
  if (legend) {
    ...
    legend(
      x = 1, y = 1, argnames, xjust = .5, yjust = .5,
    ...
    if (any(arrayres != 0.5)) {
      legend(
        x = 0, y = 0 - (.44 * min(1:subleg)), yjust = 0,
        bty = "n",
        text.col = "gray40",
        title = "Restrictions:",
        title.adj = 0,
