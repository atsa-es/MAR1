fun.restrict.BC <- function(namesvar, namescovar) {
  ...
  if (length(drivers) > 0) {
    ...
    line1.covar <- "tkgrid(tklabel(frame.covar,text=colwidth))"
  }
  ...
  for (i in 1:length(taxa)) {
    taxa.labs <- paste(taxa.labs, paste("tklabel(frame.var,text=\"", taxa[i], "\")", sep = ""), sep = ",")
  } # taxa names
  ...
  if (length(drivers) > 0) {
    ...
    tkgrid(tklabel(frame.covar, text = "NA"))
  }
  ...
  tkgrid(
    tklabel(frame.overall, text = ""), tklabel(frame.overall, text = "Variates", font = f.tit),
  ...
  if (length(drivers) > 0) {
    indexCGlobal <- matrix(res.covar, nrow = length(taxa), ncol = length(drivers), byrow = T)
  } else {
    indexCGlobal <- NULL
  }
