summary.MAR <- function(object, ...) {
  ...
  stab.best <- data.frame(
    Attribute = c(rep("resilience", 3), rep("reactivity", 2)),
  ...
  if (!is.null(object$bootstrap)) {
    ...
    ics.boot <- c(object$bootstrap$AIC, object$bootstrap$BIC)
    names(ics.boot) <- c("AIC", "BIC")
  ...
  msummary <- list(
    coef.summary.best = coef.sum.best, coef.summary.boot = coef.sum.boot,
